...
while r:
    ...
    if time.time()-player.specialtime<0.5:
        ...
        for josh in joshs:
            for item in josh.enemies1:
                ...
                if item.rect.colliderect(player.rect):
                    player.health+=1
        pygame.draw.circle(screen,lightblue,(player.rect.x+25,player.rect.y+25),125,0)
    if display:
    ...
    for enemy in enemies:
        ...
        for enemyy in enemies:
            if enemy!=enemyy:
                if enemy.shot.colliderect(enemyy.rect):
                    enemies.remove(enemyy)
                    enemy.shot.y=2000
        for aarron in aarrons:
            if enemy!=aarron:
                if enemy.shot.colliderect(aarron.rect):
                    aarron.health-=1
                    enemy.shot.y=2000
        for josh in joshs:
            if enemy!=josh:
                if enemy.shot.colliderect(josh.rect):
                    josh.health-=1
                    enemy.shot.y=2000
        if display:
        ...
        for enemy in enemies:
            if shot.colliderect(enemy.rect):
                ...
                try:
                    ...
                    pass
        for aarron in aarrons:
            if shot.colliderect(aarron.rect):
                aarron.health-=1
                try:
                    player.shots.remove(shot)
                except:
